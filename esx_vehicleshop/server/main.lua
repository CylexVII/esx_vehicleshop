...
AddEventHandler('esx_vehicleshop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = vehicleModel.. ' aracını yarı fiyatına sattın', length = 2500})
		else
...
RegisterCommand('araçtransfer', function(source, args)
            ...
            if result[1] ~= nil then
                ...
                if myID == CarOwner then
					...
					data = {}
                        ...
                        TriggerClientEvent('chatMessage', other, "" .. plate .. "^r^4was transfered to you by: ^*^2" .. myName)
                    ...
                    TriggerClientEvent('chatMessage', source, "^*^1Bu araca sahip değilsin")
                end
            else
